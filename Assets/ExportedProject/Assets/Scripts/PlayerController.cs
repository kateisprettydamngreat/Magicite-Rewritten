...
public class PlayerController : MonoBehaviour
{
    ...
    public virtual void Update()
    {
        ...
        if (!GameScript.takingDamage)
        {
            ...
            else if (!(Camera.main.ScreenToWorldPoint(UnityEngine.Input.mousePosition).x >= t.position.x - 0.1f) &&
                     !GameScript.facingLeft)
            {
                GameScript.facingLeft = true;
    ...
    public virtual void UpdateAppearance()
    {
        ...
        if (helm == 0)
        {
            head2.GetComponent<Renderer>().material =
                (Material)Resources.Load("h/h" + (h + (MenuScript.pColor - 1) * 14), typeof(Material));
        }
        else
        {
