...
public class SpawnScript : MonoBehaviour
{
	...
	public override void SpawnTown()
	{
		...
		for (num = 0; num < slot.Length - 2; num += 3)
		{
			...
			{
				if (Network.isServer && num != 9 && num != 10)
				{
	...
	public override void SpawnChest(Vector3 pos)
	{
		...
		if (MenuScript.multiplayer > 0)
		{
			if (Network.isServer)
			{
				if (Random.Range(0, 8) == 0)
				{
					GameObject val = (GameObject)Network.Instantiate(Resources.Load("interact/chest2"), pos, Quaternion.identity, 0);
				}
				else
				{
					GameObject val2 = (GameObject)Network.Instantiate(Resources.Load("interact/chest"), pos, Quaternion.identity, 0);
				}
			}
		}
		else
		{
			GameObject val3 = (GameObject)Object.Instantiate(Resources.Load("interact/chest"), pos, Quaternion.identity);
			val3.transform.parent = ((Component)this).transform;
		}
	}
